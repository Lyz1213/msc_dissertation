...
def main():
    ...
    for fn_model in glob.glob(opt.model_path):
        ...
        translator = table.TransformerTranslator(opt, dummy_opt.__dict__)
        ...
        for metric_name in metric_name_list:
            ...
            b_correct = sum((x.bleu[metric_name] for x in r_list))
            b1_correct = sum((x.bleu1[metric_name] for x in r_list))
            b2_correct = sum((x.bleu2[metric_name] for x in r_list))
            b3_correct = sum((x.bleu3[metric_name] for x in r_list))
            b4_correct = sum((x.bleu4[metric_name] for x in r_list))
            print('BLEU score: {}, 1: {}, 2: {}, 3: {}, 4: {}'.format((float(b_correct)/float(len(r_list))), 
            (float(b1_correct)/float(len(r_list))), (float(b2_correct)/float(len(r_list))), (float(b3_correct)/float(len(r_list))), (float(b4_correct)/float(len(r_list)))))
        ...
        for template in template_js_list:
            if len(template_js_list[template]) != 0:
                ...
                for batch in test_data:
                    ...
                    r_list += r
                r_list.sort(key=lambda x: x.idx)
                assert len(r_list) == len(template_js_list[template]), 'len(r_list) != len(js_list): {} != {}'.format(
                    len(r_list), len(template_js_list[template]))

                # evaluation
                for pred, gold in zip(r_list, template_js_list[template]):
                    pred.eval(gold)
                print('Results:')
                for metric_name in metric_name_list:
                    c_correct = sum((x.correct[metric_name] for x in r_list))
