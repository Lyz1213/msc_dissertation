...
class Translator(object):
    ...
    def run_lay_decoder(self, decoder, classifier, q, q_all, q_enc, max_dec_len, vocab_mask, vocab):
        ...
        for i in range(max_dec_len):
            ...
            #print('dec_all {} dec_state {}'.format(dec_all.size(), dec_state))
    ...
    def run_tgt_decoder(self, embeddings, tgt_mask_seq, lay_index_seq, lay_all, decoder, classifier, q, q_all, q_enc,
                        max_dec_len, lay_skip_list, vocab, copy_to_ext, copy_to_tgt):
        batch_size = q.size(1)
    ...
    def translate(self, batch):
        ...
        lay_dec = self.run_lay_decoder(
            self.model.lay_decoder, self.model.lay_classifier, q, q_all, q_enc, self.opt.max_lay_len, vocab_mask,
            self.fields['lay'].vocab)
        #print('lay_dec', lay_dec.size())
        #print('oracle', batch.lay[0].data[1:].size())
        #lay_dec = batch.lay[0][1:]
        #print('lay_dec', lay_dec.size())
        ...
        # target decoding
        tgt_dec = self.run_tgt_decoder(self.model.tgt_embeddings, tgt_mask_seq, lay_index_seq, lay_all,
                                       self.model.tgt_decoder,
                                       self.model.tgt_classifier, q, q_tgt_all, q_tgt_enc, self.opt.max_tgt_len,
                                       lay_skip_list, self.fields['tgt_not_copy'].vocab, batch.copy_to_ext, batch.copy_to_tgt)
        # recover target
        tgt_list = []
        for b in range(batch_size):
            tgt = recover_target_token(lay_skip_list[b], tgt_dec[:, b], self.fields['tgt_not_copy'].vocab,
                                       self.fields['copy_to_ext'].vocab, tgt_dec.size(0))
            tgt_list.append(tgt)
