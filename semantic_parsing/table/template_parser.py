...
class Parser:
        ...
        if all_nns:
            for token in doc:
                ...
                if token.head.i == token.i + 1:
                    ...
                    if token_tag in pos_mapper:
                        mapped_terms.append(
                            [pos_mapper[token_tag], (token.text + ' ' + token.head.text).strip(), token_tag])
    ...
    def create_alist(self, alist, _attr_value, var_ctr, tokens):
        ...
        regex_patterns = [
            # op X of Y in T
            (10,
             '^(?P<op>operation-\d*) (?P<prop>propclass-\d*) (of)-\d* (?P<entity>entity-\d*) (in|at)-\d* (?P<time>datetime-\d*)$'),
            # X of Y in T
            ...
            (30,
             '^(?P<op>operation-\d*) (?P<prop>propclass-\d*) (of)-\d* (?P<entity>.*) (in|at)-\d* (?P<time>datetime-\d*)$'),
            # prop of (nested entity in T )
            (32,
             '^(?P<prop>propclass-\d*) (of)-\d* (?P<entity>(propclass-\d* (in|at)-\d*.*) (in|at)-\d* (datetime-\d*))$'),
            # X of <nested Y in T
            ...
            (50,
             '^(?P<op>operation-\d*) (?P<prop>propclass-\d*) (in|at)-\d* (?P<time>datetime-\d*) (of)-\d* (?P<entity>.*)$'),
            # X in T of Y
            ...
            (70,
             '^(?P<entity>.*) (with|verb)-\d* (?P<op>operation-\d*) (?P<prop>propclass-\d*) (in|at)-\d* (?P<time>datetime-\d*)$'),
        ...
        for attr, attr_val in kv_pairs.items():
                        ...
                        for group_name, matched_str in m.groupdict().items():
                            for k, v in curr_alist.items():
                                ...
                                if v == ('@' + group_name):
                                    ...
                                    if len(token_idx) == 2 and not (
                                            matched_pattern in [70, 75] and group_name == "entity"):
                                        curr_alist[k] = tokens[int(token_idx[1])][1]
                                    else:
                                        curr_alist[k] = "%" + matched_str

                                    # if prop is plural, replace VALUE with VALUES op
                                    if v == '@prop' and curr_alist['h'] == 'value' and tokens[int(token_idx[1])][
                                        2] == 'NOUN:NNS':
                                        curr_alist['h'] = 'values'
                                    break
                                elif k == '$filter':
            ...
            if 'h' in curr_alist and curr_alist['h'] not in ['value', 'values']:
                if curr_alist['h'] not in operator_mapping:
                    #print('something happened')
                    return(None, var_ctr, matched_pattern)
