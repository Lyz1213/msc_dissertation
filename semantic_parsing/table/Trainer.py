...
def _debug_batch_content(vocab, ts_batch, gold_batch):
    print('gold batch ', gold_batch)
    print('ts batch', ts_batch)
    ...
    for b in range(batch_size):
        ...
        for i in range(seq_len):
            ...
            tk_list.append(tk)
            gtk = vocab.itos[gold_batch[i,b]]
            gold_list.append(gtk)
        print('lay gold {} \n lay pred {}'.format(gold_list, tk_list))
...
class Trainer(object):
    ...
    def forward(self, epoch, batch, criterion, fields):
        ...
        q_batch = [_debug_seq(fields['src'].vocab, seq) for seq in q]
