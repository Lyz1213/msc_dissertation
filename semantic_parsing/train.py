...
def build_optim(model, checkpoint):
    ...
    if opt.model_name == 'transformer':
        bert_params = list(map(id, model.q_encoder.model.parameters()))
        base_params = filter(lambda p: id(p) not in bert_params,
                             model.parameters())
        param_dict = {'all':model.parameters(), 'base': base_params, 'pretrain': model.q_encoder.model.parameters()}
    else:
        param_dict = {'all':model.parameters(), 'base': model.parameters()}
    optim.set_parameters(param_dict)
...
def main():
    ...
    # Build model.
    if(opt.model_name == 'transformer'):
        model = build_transformer_model(model_opt, fields, checkpoint)
    else:
