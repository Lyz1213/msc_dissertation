...
import table.BARTModelConstrunctor
import table.Models
...
class TransformerTranslator(object):
    def __init__(self, opt, dummy_opt={}):
        ...
        self.model = table.BARTModelConstrunctor.make_bart_base_model(
    ...
    def self_translate_lay(self, src,attention_mask, batch_size):
        encoder = self.model.enc_dec1.get_encoder()
        decoder = self.model.enc_dec1.get_decoder()
        ...
        for i in range(80):
            ...
            tgt_outs = self.model.enc_dec1(attention_mask = attention_mask, encoder_outputs=encoder_out,
    ...
    def translate(self, batch):
        ...
        bart_tgt = batch.bart_tgt.transpose(0, 1)
        bart_lay_train = batch.bart_lay_train.transpose(0, 1)
        bart_lay_test = batch.bart_lay_test.transpose(0, 1)
        # lay_index = lay_index.transpose(0,1)
        # tgt_mask = tgt_mask.transpose(0,1)
        tgt_loss = batch.bart_tgt_loss.transpose(0, 1)
        dec_inp = tgt_loss[:, :-1]
        ...
        if self.opt.model_type == 'onestage':
            ...
            tgt_list, encoder_out = self.self_translate_lay(bart_src, attention_mask, batch_size)
            #lay_output = self.model.enc_dec1.generate(bart_src, return_dict_in_generate=True, output_hidden_states=True)
            #self.self_translate_lay(bart_src, attention_mask)
            #lay_out = lay_output.sequences.transpose(0, 1)
            #encoder_outs = lay_output.encoder_hidden_states
            #print('encoder_out', encoder_outs[-1].size())
            #encoder_out = (encoder_outs[-1],)
            
            # encoder_out = lay_out.encoder_last_hidden_state
            ...
            #for b in range(batch_size):
            for tgt in tgt_list:
                #generated = self.vocab.tokenizer.convert_ids_to_tokens(lay_out[:, b])
                #print('generated is ', generated)
                #modified_lay, lay_skip, lay = modify_lay(tgt)
                modified_lay, lay_skip, lay = modify_lay_(tgt)
                #print('modified {} \n lay_skip {}\n lay{}\n'.format(modified_lay, lay_skip, lay))
                #print('lay_skip is ', lay_skip)
                lay_list.append(lay)
                lay_out_list.append(self.vocab.tokenizer.convert_tokens_to_ids(modified_lay))
                lay_skip_list.append(lay_skip)
                #print('modified is {}\nlay_skip is {}\n'.format(modified_lay, lay_skip))
                lay_index_list.append(table.IO.get_lay_index(lay_skip, modified_lay))
                tgt_mask_list.append(table.IO.get_tgt_mask(lay_skip))
            #print('lay_out ', lay_out_list)
            if self.opt.oracle_sketch:
                tgt_mask_seq = batch.tgt_mask
                lay_index_seq = batch.lay_index
                lay_inp = bart_lay_test
            else:
                tgt_mask_seq = add_pad(tgt_mask_list, 0).float().t()
                lay_index_seq = add_pad(lay_index_list, 0).t()
                #if(lay_index_seq.size() == batch.lay_index.size()):
                #lay_inp = bart_lay_test
                lay_inp = add_pad(lay_out_list, 1).t().transpose(0, 1)
                #print('lay index {}\ntrue index {}'.format(lay_index_seq, batch.lay_index))
            #print('lay_inp ',lay_inp.size())
            lay_encoder = self.model.enc_dec2.get_encoder()
            tgt_decoder = self.model.enc_dec2.get_decoder()
            lay_encode = lay_encoder(lay_inp)
            lay_encode = lay_encode.last_hidden_state.transpose(0,1)
            #print('tgt_mask_seq ', tgt_mask_seq.size())
            #print('lay_index', lay_index_seq.size())
            #print('lay_encode', lay_encode.size())
            inp = torch.LongTensor(1, batch_size).fill_(table.IO.BOS).cuda()
            batch_index = torch.LongTensor(range(batch_size)).unsqueeze_(0).cuda()
            for i in range(lay_index_seq.size(0)):
                lay_index = lay_index_seq[:i+1,:]
                #print('lay_index ', lay_index.size())
                lay_select = lay_encode[lay_index, batch_index, :]

                # inp.masked_fill_(inp.ge(len(tgt_not_copy_vocab)), table.IO.UNK)
                inp = inp.transpose(0,1)
                tgt_inp_emb = tgt_decoder.embed_tokens(inp) * tgt_decoder.embed_scale
                tgt_inp_emb = tgt_inp_emb.transpose(0,1)
                tgt_mask_expand = v_eval(tgt_mask_seq[:i+1,:].unsqueeze(2).expand_as(tgt_inp_emb))
                #print('mask_expand', tgt_mask_expand.size())
                tgt_inp = tgt_inp_emb.mul(tgt_mask_expand) + \
                          lay_select.mul(1 - tgt_mask_expand)
                #print('lay_select {}, tgt_inp_emb {}, tgt_mask_expand {}, inp {}'.format(lay_select.size(), tgt_inp_emb.size(), tgt_mask_expand.size(), tgt_inp.size()))
                tgt_inp = tgt_inp.transpose(0, 1)
                #print('tgt_inp size ', tgt_inp.size())
                #print('encoder_outputs ', encoder_out.size())
                tgt_outs = self.model.enc_dec2(attention_mask = attention_mask, encoder_outputs=encoder_out,
                                        decoder_inputs_embeds=tgt_inp)
                tgt_out = tgt_outs.logits.transpose(0,1)
                #print('tgt_out ',tgt_out.size())
                next_words = argmax(tgt_out)
                #print('next_words', next_words.size())
            ...
            for b in range(batch_size):
                tgt = self.vocab.tokenizer.convert_ids_to_tokens(inp[:,b])
                tgt = modify_tgt(tgt)
                #Hv = lay_list[b][0] + 'h' + lay_list[b][1].replace("Ġ","")
                Hv = lay_list[b][0]
                if tgt[0] != Hv:
                    tgt[0] = Hv
                tgt_list.append(tgt)
